...
class Connector extends Events {
     ...
     * @param {boolean} [config.ngrok] - set up ngrok for Listener?
    ...
    async startConversation (externalConsumerId, userProfile) {
        ...
        if (umsResponse[1].code === 'BAD_REQUEST') {
            ...
            throw '[connector.startConversation] conversation not started, no userId extracted'
        }
    }
    ...
    async openConversation (externalConsumerId, userProfile) {
        try {
            return await this.startConversation(externalConsumerId, userProfile);
        } catch (e) {
            if (e.userId) {
                let conversation = e;
 ...
 */
class Conversation {
    constructor(externalConsumerId, userProfile) {
...
/**
 * @typedef {Object} MIAPIResponse
 * @property {Object} _metadata
 * @property {number} _metadata.count
 * @property {Object[]} conversationHistoryRecords
 * @property {{conversationId: string}} conversationHistoryRecords[].info
 */